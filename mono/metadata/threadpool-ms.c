...
// android does not have cabs alias
#include <stdlib.h>
...
hill_climbing_update (gint16 current_thread_count, guint32 sample_duration, gint32 completions, gint64 *adjustment_interval)
{
	...
	if (sample_count > hc->wave_period) {
		...
		if (average_throughput > 0 && average_thread_count > 0) {
			...
			throughput_error_estimate = sqrt (mono_double_complex_scalar_div (hill_climbing_get_wave_component (hc->samples, sample_count, adjacent_period_1), average_throughput));

			if (adjacent_period_2 <= sample_count) {
				throughput_error_estimate = MAX (throughput_error_estimate, sqrt (mono_double_complex_scalar_div (hill_climbing_get_wave_component (
			...
			if (sqrt (thread_wave_component) > 0) {
			...
			if (noise_for_confidence > 0) {
				confidence = sqrt (thread_wave_component) / noise_for_confidence / hc->target_signal_to_noise_ratio;
			} else {
				/* there is no noise! */
				confidence = 1.0;
			}
